...
returns the events completed by those bytes, and retains only bounded
parser state (unfinished message payload, an unfinished control payload and
up to 14 header bytes). The public whole-capture entry and the legacy
regression tests are unchanged.
...
decoder one byte at a time and prints events as each push completes them.
